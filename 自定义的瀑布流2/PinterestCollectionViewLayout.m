//
//  PinterestCollectionViewLayout.m
//  CollectionViewDemo3
//
...
@implementation PinterestCollectionViewLayout
...
-(void)prepareLayout{
    ...
    _innerSpaceing = (width-self.self.columnCount*self.itemWidth)/(self.columnCount-1);
    ...
    for (int index = 0; index < _itemCount; index++) {
        ...
        //计算x坐标
        CGFloat xOffSet = _sectionInset.left + colIndex*self.itemWidth + colIndex*_innerSpaceing;
        //计算y坐标
        CGFloat yOffset = _sectionInset.top + [_itemHeightArray[colIndex] floatValue] + _innerSpaceing;
        
        //高度随机值
        CGFloat itemHeight = arc4random()%100*2+100;
        
        //更新对应列的高度
        _itemHeightArray[colIndex] = [NSNumber numberWithFloat:yOffset + itemHeight];
        
        //frame = (x,y,width,height)
        CGRect rect = CGRectMake(xOffSet, yOffset, self.itemWidth, itemHeight);
